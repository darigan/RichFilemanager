...
--------------------------------------------------------------------------------------------------------------*/
	...
	export function write(message: string, obj?: Options) {
		let log = alertify;
	...
	export function warning(message: string, options?: Options) {
		return fm.write(message, $.extend({}, {
			type: 'warning',
			delay: 10000
		}, options));
...
--------------------------------------------------------------------------------------------------------------*/
	...
	function performInitialRequest() {
		...
		}).then((response: any) => response.errors ? $.Deferred().reject() : undefined);
	}
	...
	function includeAssets(callback) {
		let primary = [],
			secondary = [];
	...
	class LangModel {
		...
		private translationsHash = {};
		private translationsPath = fm.settings.baseUrl + '/languages/';
	...
	function FmModel() {
		...
		this.fetchSelectedItems = function (instanceName) {
			let selectedNodes, selectedItems;
		...
		let TreeModel = function () {
			...
			let expandFolderDefault = function (parentNode) {
				if(fullexpandedFolder !== null) {
					...
					let node = tree_model.findByFilter(function (node) {
			...
			this.createNode = function (resourceObject) {
				let node = new TreeNodeModel(resourceObject);
		...
		let ItemsModel = function () {
			...
			this.createObject = function (resourceObject) {
				let item = new ItemObject(resourceObject);
			...
			this.getSelected = function () {
				let selectedItems = items_model.findByFilter(function (item) {
		...
		let HeaderModel = function () {
			...
			this.switchLang = function (e) {
				...
				if(langNew && langNew.toLowerCase() !== langCurrent.toLowerCase()) {
					let newUrl, url = window.location.toString(),
		...
		let RenderModel = function () {
			...
			let MarkdownRenderer = function () {
				...
				let instance = window.markdownit({
					// Basic options:
					html: true,
					linkify: true,
					typographer: true,
		...
		let DragAndDropModel = function () {
			...
			function isDropAllowed(targetItem) {
				let matches = $.grep(drag_model.items, function (itemObject, i) {
					if(targetItem.rdo.type === "folder" || targetItem.rdo.type === "parent") {
...
---------------------------------------------------------*/
	...
	function formatBytes(bytes, round) {
		...
		let d = parseFloat(round ? 1000 : 1024);
		let c = 0;
	...
	function formatServerError(errorObject) {
		let message;
	...
	function extendRequestParams(method, parameters) {
		let methodParams,
	...
	function buildAjaxRequest(method, parameters): JQuery.jqXHR {
		return $.ajax({
			type: method,
			cache: false,
	...
	function getFilteredFileExtensions() {
		if(_url_.param('filter')) {
			if(config.filter[ _url_.param('filter') ] !== undefined) {
				let shownExtensions = config.filter[ _url_.param('filter') ];
			}
		}
		return shownExtensions;
	}

	function buildConnectorUrl(params: URLParams): string {
		let defaults: URLParams = {
			time: new Date().getTime()
		};
		let queryParams: URLParams = $.extend({}, params || {}, defaults);
		return apiConnector + '?' + $.param(queryParams);
	}

	// Build url to preview files
	function createPreviewUrl(resourceObject, encode) {
		let previewUrl,
	...
	function createImageUrl(resourceObject, thumbnail, disableCache) {
		let imageUrl;
		if(isImageFile(resourceObject.id) &&
			...
			if(config.viewer.absolutePath && !thumbnail && resourceObject.attributes.path) {
				...
				let queryParams = {path: resourceObject.id};
	...
	function buildAbsolutePath(path, disableCache) {
	...
	function createCopyUrl(resourceObject) {
	...
	function processMultipleActions(items, callbackFunction, finishCallback) {
...
---------------------------------------------------------*/
	...
	function selectItem(resourceObject) {
	...
	function copyItem(resourceObject, targetPath) {
		return buildAjaxRequest('GET', {
			mode: 'copy',
	...
	// or choosing the "Move" contextual menu option in list views.
	function moveItem(resourceObject, targetPath) {
		return buildAjaxRequest('GET', {
			mode: 'move',
	...
	function deleteItemPrompt(objects, successCallback) {
		let objectsTotal = objects.length,
	...
	function downloadItem(resourceObject) {
		let queryParams = {
			mode: 'download',
			path: resourceObject.id
		};
	...
	function previewItem(resourceObject) {
		return buildAjaxRequest('GET', {
			mode: 'editfile',
			path: resourceObject.id
		}).done(function (response) {
	...
	function extractItem(resourceObject, targetPath) {
...
---------------------------------------------------------*/
	...
	function performAction(action, options, targetObject, selectedObjects) {
